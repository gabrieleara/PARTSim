...
#ifdef _MSC_VER
#pragma warning(                                                               \
    disable : 4290) // A function is declared using exception specification,
...
namespace MetaSim {
    ...
    /**
       ...
       - conversions to string types (with specification of the unit of
       measure).
...
#define xx_round(dbl)                                                          \
    dbl >= 0.0                                                                 \
        ...
        static void set_default_unit(Tick::unit_t d) {
            default_unit = d;
        }
    ...
    inline Tick operator-(const Tick &t) {
        return Tick(-t.v);
    }
