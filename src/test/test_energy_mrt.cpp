...
TEST_CASE("Experiment 18") {
    ...
    // REQUIRE (double(t0_big0->deadEvt.getTime()) == 5.0);
    ...
    cout << serv->getTasks().at(0)->toString() << endl;
    return;
    // REQUIRE (serv->getTasks().size() == 1);
    REQUIRE (serv->getTasks().at(0) == tos);
    REQUIRE (tos->endEvt.getTime() == Tick(7));
    REQUIRE (k->getIslandUtilization(1.0, IslandType::BIG, NULL) == 0.2 + 15.0/30.0 + 1.0 / 31.0);
    ...
    REQUIRE (double(t5->endEvt.getTime()) == 9.0);
    ...
    REQUIRE ((double)t3->endEvt.getTime() == 10.0);
    ...
    REQUIRE (k->getIslandUtilization(1.0, IslandType::BIG, NULL) == 0.2 + 15.0/30.0 + 1.0 / 31.0);
    REQUIRE (!k->getCBServer_CEMRTK_Utilization(serv, init_util, 1.0)); // is 0.2 server util or core active util? exp. server util
    REQUIRE ((double)tos2->endEvt.getTime() == 13.0);
    ...
    REQUIRE (k->getProcessorRunning(t0_big1) == cpus_big[1]);
    REQUIRE (k->getProcessorReady(t1_big1) == cpus_big[1]);
    REQUIRE ((double)t4->endEvt.getTime() == 14.0);
