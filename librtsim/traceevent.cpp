...
namespace RTSim {
    ...
    const string TraceEvent::evt_name[] = {
        ...
        "DLINE SET", "WAIT",     "SIGNAL",     "IDLE", "NAME",
        "DLINE MISS"};
    ...
    string TraceDlinePostEvent::getDescription() {
        ...
        sprintf(ctaskD, "%d", _taskD);
        sprintf(ctaskD2, "%d", _taskD2);
        return TraceTaskEvent::getDescription() + " - from " + string(ctaskD) +
               " to " + string(ctaskD2);
    }
