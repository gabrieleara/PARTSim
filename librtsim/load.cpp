...
namespace RTSim {
    ...
    Tick IATGen::getAvg(int i) const {
        return avg[i];
    }
    Tick IATGen::getMin(int i) const {
        return getAvg(i);
    }
    Tick IATGen::getMax(int i) const {
        return getAvg(i);
    }
    ...
    // Default behavior is Constant Value!
    Tick CTGen::getMin(int i) const {
        return getAvg(i);
    }
    Tick CTGen::getMax(int i) const {
        return getAvg(i);
    }
    ...
    void ConstCTGen::sinthesize(Task *t, int i) {
        ...
        t->addInstr(std::move(
            instr)); // new DeltaVar(* dynamic_cast< DeltaVar* >(get(i)))));
    }
    ...
    Tick DTGen::getMin(int i) const {
        return getAvg(i);
    };
    Tick DTGen::getMax(int i) const {
        return getAvg(i);
    };
    Tick DTGen::getAvg(int i) const {
        return va[i];
    };
    ...
    Tick OffsetGen::getMin(int i) const {
        return getAvg(i);
    };
    Tick OffsetGen::getMax(int i) const {
        return getAvg(i);
    };
    Tick OffsetGen::getAvg(int i) const {
        return va[i];
    };
    ...
    bool RandomOffsetGen::generate() {
        for (int i = 0; i < taskSet->size(); i++) {
            ...
            if (_islimit)
                mylimit = std::min(mylimit, _limit / _step);
    ...
    //     RandomTaskSetFactory::RandomTaskSetFactory(int n, double u, IATGen
    //     *g1, CTGen *g2,
    //                                                double minU, bool doIt,
    //                                                bool preciseU)
    //         throw(Exc) :
    //         _count(0),
    //         _size(n),
