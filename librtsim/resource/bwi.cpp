#include <rtsim/resource/bwi.hpp>
...
namespace RTSim {
    ...
    bool BWI::request(AbsRTTask *t, Resource *r, int n) {
        ...
        if (r->isLocked()) {
            ...
            if (!s)
                throw BaseExc(
                    "BWI operating on something that it is not a server");

            if (schedulers.find(s) == schedulers.end())
                throw BaseExc(
                    "BWI: Can't find server/scheduler correspondence");
            ...
            //    that the task is activated (and equeued) at maximum possible
            //    priority
            // 3) Modify the task to store its "scheduling" parameters, so that
            //    it can bring such parameters when moving from one scheduler to
            //    the other.
        } else {
            r->lock(t);
            ret = true;
