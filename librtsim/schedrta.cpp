...
#include <cassert>
#include <math.h>
#include <rtsim/schedrta.hpp>
#include <rtsim/sporadicserver.hpp>

namespace RTSim {

    /// Rate-monotonic ordering
    static bool less_rm(const SchedRTA::ServerInfo &s1,
                        const SchedRTA::ServerInfo &s2) {
    ...
    SchedRTA::SchedRTA(const string &name) : Entity(name) {}
    ...
    Tick SchedRTA::computeResponseTime(int i) const {
        ...
        do {
            ...
            for (int j = 0; j < i; ++j)
                // This is a ceil()
                r_new += (r_cur + servers[j].P - 1) / int(servers[j].P) *
                         servers[j].Q;
