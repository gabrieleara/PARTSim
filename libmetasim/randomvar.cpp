...
namespace MetaSim {
    ...
    std::unique_ptr<ExponentialVar>
        ExponentialVar::createInstance(vector<string> &par) {
        if (par.size() != 1)
    ...
    std::unique_ptr<WeibullVar>
        WeibullVar::createInstance(vector<string> &par) {
        if (par.size() != 2)
    ...
    DetVar::DetVar(const std::string &filename) : _array() {
